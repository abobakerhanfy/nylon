...
class _SendOrderBalanceState extends State<SendOrderBalance> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await _controllerPayment.getAllPaymentsAndAutoSelectMyFatoorah(
          customerId: customerId);
      print(_controllerPayment.paymentsDataList.map((e) => e.code));
    });
  }
                    ...
                    controller.statusRequestSendOrderB == StatusRequest.loading
                        ...
                        : ButtonOnCart(
                            ...
                            onTap: () async {
                              ...
                              await controller
                                  .sendDataUser(); // إضافة بيانات المستخدم
                              ...
                              // ✅ هنا نضيف تسجيل وسيلة الدفع أولًا لضمان وجودها بالsession
                              print(
                                  "🟢 تسجيل وسيلة الدفع ${_controllerPayment.selectCodePayment}");
                              await _controllerPayment.selectPayment(
                                paymentCode:
                                    _controllerPayment.selectCodePayment,
                              );

                              print("✅ وسيلة الدفع سجلت في الـ session");

                              if (_controllerPayment.selectCodePayment ==
                                  "myfatoorah_pg") {
                                print(
                                    "🚀 بدء عملية الدفع عبر ماي فاتورة balance");
                                await _controllerPayment
                                    .processBalancePaymentWithMyFatoorah();
                                print(
                                    "🟢 balancePaymentModel.orderId = ${_controllerPayment.balancePaymentModel?.orderId}");
                              }
                            },
                          ),
                  ],
      ...
      body: ListView(
        ...
        children: [
          ...
          GetBuilder<ControllerPayment>(builder: (controllerPayment) {
            print(
                "🟢 selectCodePayment => ${_controllerPayment.selectCodePayment}");
            print(
                "🟢 paymentsDataList => ${_controllerPayment.paymentsDataList}");
            print(
                "🟢 balancePaymentModel => ${_controllerPayment.balancePaymentModel}");
            print(
                "🟢 balancePaymentModel.orderId => ${_controllerPayment.balancePaymentModel?.orderId}");
