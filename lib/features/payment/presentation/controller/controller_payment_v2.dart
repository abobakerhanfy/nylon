...
class ControllerPayment extends PaymentController {
  ...
  Future<void> getAllPaymentsAndAutoSelectMyFatoorah(
      {String? customerId}) async {
    print("🚀 تحميل وسائل الدفع ثم اختيار ماي فاتورة تلقائياً...");
    print(
        "🧭 getAllPaymentsAndAutoSelectMyFatoorah: customerId => $customerId");
    print(
        "🚀 Calling paymentMyFatoorah with data: {order_id: ${selectPaymentModel?.orderId}}");
    ...
    return response.fold((failure) {
      ...
      if (data.containsKey("error")) {
        ...
        try {
          ...
          // ✅ لو معطى customerId، استدعي API fastCheckout
          ...
          await selectPayment(
              paymentCode: myFatoorah.code!); // 🛠 هنا يتأكد من استدعاء الAPI

          await selectPayment(paymentCode: myFatoorah.code!);

          if (myFatoorah.code == "myfatoorah_pg") {
            await processBalancePaymentWithMyFatoorah();
            print(
                "✅ بعد processBalancePaymentWithMyFatoorah order_id = ${balancePaymentModel?.orderId}");
          }
          // ✅ مباشرة افتح WebView الدفع
          await paymentMyFatoorah();
        } catch (e) {
