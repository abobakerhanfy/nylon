...
class _ViewOneProductState extends State<ViewOneProduct> {
  ...
  T? firstOrNull<T>(List<T>? xs) =>
      (xs != null && xs.isNotEmpty) ? xs.first : null;

  @override
  Widget build(BuildContext context) {
    final apiList = _controller.productModel?.product;
    final name = (apiList != null && apiList.isNotEmpty)
        ? apiList.first.name
        : (_controller.products?.name ?? '');
    return GetBuilder<ControllerOneProduct>(builder: (controller) {
      return Scaffold(
        ...
        appBar: customAppBar(
          label:
              translate(name, name)!, // نفس القيمة عربي/إنجليزي لو مفيش ترجمة
          isBack: true,
          onTap: () {
              ...
              builder: (context, boxSize) {
                return GetBuilder<ControllerOneProduct>(builder: (controller) {
                  return HandlingDataView(
                    statusRequest:
                        controller.statusRequestGetOP ?? StatusRequest.loading,
                    widget: GetBuilder<ControllerOneProduct>(
                      builder: (controller) {
                        return ListView(
                          children: [
                            SizedBox(height: boxSize.maxHeight * 0.03),
                            if ((controller.productModel?.product?.isNotEmpty ??
                                false))
                            ...
                            if ((controller
                                    .productModel?.productRelated?.isNotEmpty ??
                                false)) ...{
                              SizedBox(height: boxSize.maxHeight * 0.03),
                              SeeAll(
                                  onTap: () {},
  ...
  Widget _buildProductDetails(BuildContext context) {
    final p = (_controller.productModel?.product?.isNotEmpty ?? false)
        ? _controller.productModel!.product!.first
        : null;

    final priceAfter = _controller.productModel != null
        ? _controller.productModel!
            .calculatePriceWithDiscount(_controller.count)
        : _controller.unitPriceWithDiscount(_controller.count);
    ...
    return Row(
      ...
      children: [
        Expanded(
          child: Column(
            ...
            children: [
              ...
              if ((_controller.productModel?.discounts?.isNotEmpty ??
                  false)) ...[
        ...
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if ((p?.special != null && p!.special != 0 && (p.price ?? 0) != 0))
              Row(
                children: [
                  Text(
                    p.special!.toStringAsFixed(2),
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                  ...
                  Image.asset("images/riyalsymbol_compressed.png", height: 14),
                ],
              ),
            Row(
              children: [
  ...
  Widget _buildProductDescription(
    ...
    // خُد الوصف الخام من الـ API (الداتا بيز جاية عبر الـ API عندك)
    final rawHtml = _controller.productModel?.product?.first.description ?? '';

    // نطبّع النص: ننزل سطر بعد النقطة ونبدّل الـ &nbsp; بـ <br/>
    ...
    return Column(
      ...
      children: [
        ...
        // عرض الوصف كرندر HTML (flutter_html)
        Html(
        ...
        if ((_controller.productModel?.discounts?.isNotEmpty ?? false))
          Center(
            child: Container(
